...
import { useTvQuery } from '~/composables/useTvQuery.ts'
import { useTvData } from '~/composables/useTvData.ts'

const { data: genresData } = await useFetch('/api/genres', { query: { type: 'tv' } })
...
const {
  ...
  setPage,
  clearSearch,
} = useTvQuery({ debounceMs: 500 })

const { data, pending } = useTvData({
  searchQuery,
  currentPage,
  selectedGenreId,
  currentSort,
  ...
  <div class="px-8 pt-8">
    <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
      <h1 class="text-3xl font-bold">
        {{ searchQuery ? `Hledám seriál: ${searchQuery}` : 'Co bude dnes k obědu?' }}
      </h1>
    ...
    <div class="flex flex-wrap gap-4 mb-4">
      ...
      <div v-if="!searchQuery" class="flex">
        <SortBy :model-value="currentSort" @update:model-value="setSort" />
      </div>
    </div>

    <div v-if="pending" class="text-center py-20">Hledám seriály...</div>
